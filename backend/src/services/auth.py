...
class AuthService:
    ...
    async def register(self, content: UserCreate) -> AuthTokens:
        ...
        new_user = User(username=content.username, password_hash=hashed_password)
    ...
    async def login(self, content: UserCreate) -> AuthTokens:
        ...
        if user is None or not self.password_service.verify_password(
            content.password, user.password_hash
        ):
    ...
    async def logout(self, access_raw: str | None, refresh_raw: str | None):
        ...
        if access_raw is not None:
            try:
                access_token = await self._validate_token_type(
                    access_raw, TokenType.ACCESS
                )
    ...
    async def _validate_token_type(
        self, encoded_token: str, expected_type: TokenType
    ) -> Token:
        token = await self.token_service.decode_and_validate(encoded_token)
        if token.token_type != expected_type:
            raise HTTPException(
                401, f"Expected {expected_type}, found {token.token_type}"
            )
        return token
...
async def authenticate(
    ...
    try:
        ...
        if isinstance(conn, WebSocket):
            await conn.close(code=status.WS_1008_POLICY_VIOLATION, reason=e.detail)
