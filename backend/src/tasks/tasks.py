...
async def update_status(
    redis: Redis, task_id: str, status: TaskStatus, data: str | None = None
):
...
async def stt_step(
    bucket: str, filename: str, stt_service: STTService = TaskiqDepends(get_stt_service)
) -> dict[Any, Any]:
...
@broker.task(retry_on_error=True)
async def llm_step(
    text: str, llm_service: LLMService = TaskiqDepends(get_llm_service)
) -> str:
...
async def save_step(
    ...
    lecture = await lecture_crud.create(
        Lecture(user_id=user_id, name=filename, audio_url=filepath, text=text)
    )
...
async def run_audio_pipeline(
    ...
    try:
        ...
        stt_result = (
            await (await stt_step.kiq(bucket, filename)).wait_result()
        ).return_value
        ...
        if not stt_result or "text" not in stt_result:
            await update_status(
                ...
                task_id,
                TaskStatus.ERROR,
                "Ошибка: STT сервис не смог обработать аудио",
            )
        ...
        summary = (
            await (await llm_step.kiq(stt_result["text"])).wait_result()
        ).return_value

        await update_status(redis, task_id, TaskStatus.SAVING, summary)
        lecture_id = (
            await (
                await save_step.kiq(user_id, filename, filepath, summary)
            ).wait_result()
        ).return_value
