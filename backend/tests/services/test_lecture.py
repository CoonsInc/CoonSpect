...
from src.api.schemas.lecture import LecturesPage, LectureRead
...
# --- Тесты для get_lectures_page ---

@pytest.mark.asyncio
async def test_get_lectures_page_success(lecture_service, mock_lecture_crud, sample_lecture):
...
async def test_service_get_audiolink_success(
    ...
    # Используем ANY для игнорирования TTL аргумента
    mock_s3_service.get_download_url.assert_called_once_with("my-bucket", "audio.mp3", ANY)

@pytest.mark.asyncio
async def test_service_get_audiolink_no_audio(lecture_service, mock_lecture_crud, sample_lecture):
