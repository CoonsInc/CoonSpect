...
async def test_create_user_success(
    ...
    # Проверяем базовый метод create через UserCRUD
    user_data = {
        "username": "new_user",
        "password_hash": "hashed_string_123"
    }
...
async def test_read_by_username_exists(
    ...
    # 1. Сначала создаем пользователя вручную
    username = "unique_bob"
    new_user = User(
        id=uuid4(), 
        username=username, 
    ...
    # 2. Тестируем специфичный метод UserCRUD
...
@pytest.mark.asyncio
async def test_read_by_username_not_found(
    user_crud: UserCRUD
) -> None:
...
async def test_read_user_by_id(
    ...
    # Проверяем наследуемый метод read
...
async def test_update_user_password(
    ...
    assert updated_user.username == "bob" # не изменился
...
async def test_update_user_identity(
    ...
    # Проверяем, что в базе имя тоже обновилось
