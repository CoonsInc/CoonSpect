...
async def sample_lecture(db_session: AsyncSession, sample_user: User) -> Lecture:
    lecture = Lecture(
        ...
        name="Base Lecture", 
        text="Content", 
        audio_url="bucket/test.mp3", # Добавляем URL для тестов аудио-линков
...
@pytest.mark.asyncio
async def test_get_lectures_list_pagination(client: AsyncClient, db_session: AsyncSession, sample_user: User) -> None:
    for i in range(5):
...
@pytest.mark.asyncio
async def test_edit_lecture_success(client: AsyncClient, sample_user: User, sample_lecture: Lecture, authorize_override) -> None:
    authorize_override(sample_user)
    response = await client.patch(f"/lecture/edit/{sample_lecture.id}", json={"name": "Updated"})
    assert response.status_code == status.HTTP_200_OK
    assert response.json()["name"] == "Updated"

@pytest.mark.asyncio
async def test_delete_lecture_success(client: AsyncClient, sample_user: User, sample_lecture: Lecture, authorize_override) -> None:
    authorize_override(sample_user)
    response = await client.delete(f"/lecture/delete/{sample_lecture.id}")
    assert response.status_code == status.HTTP_200_OK
    assert response.json()["status"] == "success"

# --- Тесты для аудио-линков ---

@pytest.mark.asyncio
async def test_get_audiolink_route_success(client: AsyncClient, sample_lecture: Lecture) -> None:
    ...
    with patch("src.services.s3.S3Service.get_download_url", return_value=fake_url):
        ...
        # Если ты используешь стандартную схему, где данные лежат в 'details':
        if "details" in data:
            assert data["details"] == fake_url
        elif "message" in data:
            assert data["message"] == fake_url
        else:
            # Если данные пришли в другом ключе, этот принт покажет в логах pytest, в каком именно
            print(f"\nActual response data: {data}")
            # На крайний случай проверяем, есть ли значение вообще среди значений словаря
...
async def test_delete_audiolink_route_success(
    ...
    with patch("src.services.s3.S3Service.delete", return_value=None):
        ...
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        # Тут response_model=LectureRead, так что проверяем поля лекции
        assert data["id"] == str(sample_lecture.id)
        assert data["audio_url"] is None

@pytest.mark.asyncio
async def test_delete_lecture_forbidden(client: AsyncClient, db_session: AsyncSession, authorize_override) -> None:
    # Создаем чужую лекцию
    other_user = User(id=uuid4(), username="other", password_hash="hash")
    foreign_lecture = Lecture(id=uuid4(), name="Foreign", text="...", user_id=other_user.id)
